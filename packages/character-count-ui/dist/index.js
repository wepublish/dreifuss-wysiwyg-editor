...
const getTextString = editor => {
  ...
  return [...slate.Node.texts(editor)].reduce((string, nodePair, index) => {
...
const calculateCharCount = editor => {
  if (!editor) return 0; // using lodash toArray to get correct length for characters like emojis

  return toArray_1(getTextString(editor)).length;
};

function getCharacterCount(id) {
  const editor = plateCore.useStoreEditorState(id);
  return calculateCharCount(editor);
}
const CharCountToolbar = ({
  id
}) => {
