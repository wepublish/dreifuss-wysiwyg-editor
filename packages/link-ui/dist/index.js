...
const LinkElement = props => {
  ...
  return (
    ...
    // @ts-ignore
    jsxRuntime.jsx(_StyledA, { ...attributes,
      href: element.url,
...
const ToolbarLink = () => {
  ...
  const [prefix, setPrefix] = react.useState(prefixType.http);
  const [isValidURL, setIsValidURL] = react.useState(false);
  const [isInsertBtnDisabled, setIsInsertBtnDisabled] = react.useState(false);
  const {
    toggleMenu
  } = react.useContext(common.ModalContext);
  react.useEffect(() => {
    if (url) {
      link.validateUrl(url).then(value => setIsValidURL(value));
  ...
  react.useEffect(() => {
    ...
    const nodes = Array.from(slate.Editor.nodes(editor, {
      ...
      match: node => slate.Element.isElement(node) && node.type === plateCore.getPlatePluginType(editor, link.ELEMENT_LINK)
    }));
    const tuple = nodes[0];
    ...
    if (tuple) {
      ...
      const text = slate.Editor.string(editor, editor.selection);
    ...
    children: [/*#__PURE__*/jsxRuntime.jsxs("div", {
      ...
      children: [/*#__PURE__*/jsxRuntime.jsx("button", {
        ...
        onClick: e => {
          if (!editor) return;
          e.preventDefault();
          link.upsertLinkAtSelection(editor, {
